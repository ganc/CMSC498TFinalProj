...
for(i in c("corrplot", "car", "dplyr", "tidyr", "ggplot2", "gridExtra")){
...
What about correlations between family/home attributes? Any strong correlations between such variables can lead to misleading estimates when modeling the data. Since many of these attributes are binary, we will try to encode the data as 0 and 1.
...
None of the binary data showed significant correlation to one another. It looks like the only candidate for a strong correlation is the relationship between the levels of education for each parent. When modeling the data, perhaps only one of these variables should be used.
...
Parental Education Levels and Employment
----------------------------------------

```{r}
mom_edu_finals <- data %>% 
  mutate(mom_edu_fac = factor(mom_edu, levels=seq(0,4), labels=c("None", "4th grade","5th-9th\ngrade", "Secondary\neducation", "Higher\neducation"), ordered=TRUE)) %>% 
  group_by(mom_edu_fac) %>% 
  summarise(mean_est = t.test(final_grade)$estimate, left_CI = t.test(final_grade)$conf.int[1], right_CI = t.test(final_grade)$conf.int[2]) 
...
mom_edu_finals_plot <- ggplot(mom_edu_finals, aes(x=mom_edu_fac)) + 
  ...
  labs(title="Estimated Mean Final Grade\nand 95% Confidence Interval\nby Mother's Education Level", x="Mother's Education Level", y="Estimated Mean Final Grade") + 
  ylim(5,15) + 
  theme(axis.text.x=element_text(angle=50, vjust=0.5))

dad_edu_finals = data %>% 
  mutate(dad_edu_fac = factor(dad_edu, levels=seq(0,4), labels=c("None", "4th grade","5th-9th\ngrade", "Secondary\neducation", "Higher\neducation"), ordered=TRUE)) %>% 
...
dad_edu_finals_plot <- ggplot(dad_edu_finals, aes(x=dad_edu_fac)) + 
  geom_point(aes(y=mean_est), size=3, color='darkgreen') + 
  geom_errorbar(aes(ymin=left_CI, ymax=right_CI), width=0.1) + 
  labs(title="Estimated Mean Final Grade\nand 95% Confidence Interval by Father's Education Level", x="Father's Education Level", y="Estimated Mean Final Grade") + 
  ylim(5,15) + 
  theme(axis.text.x=element_text(angle=50, vjust=0.5))
