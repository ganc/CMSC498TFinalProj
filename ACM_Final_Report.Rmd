...
Our final model has an adjusted r-squared of `r summary(fit3)$adj.r.squared`. While it is lower than the all encompassing model's adjusted r-squared of `r summary(fitall)$adj.r.squared`, it only depends on `r 2**3 + 5` terms instead of `r 2**30`. It captures `r summary(fit3)$adj.r.squared/summary(fitall)$adj.r.squared` of the variation explained by the complete model, and is far more interpretable. 
...
ggplot(data_ffit_res, aes(x = final_grade, y = fitted)) + geom_point(alpha=0.2) + geom_abline(intercept=0, slope=1, colour="red")
```
